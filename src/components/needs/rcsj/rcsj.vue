        ...
        <div class="content">
          ...
          <div class="table-list">
            <el-table :data="table.tableData" border style="width: 100%" v-if="table.tableHeight"
              ...
              <el-table-column sortable prop="finish_time" label="期望完成日期" width="120"></el-table-column>
                ...
                <div class="console-tab-content">
                  <el-form label-width="150px" label-position="left">
                    <el-row :gutter="20">
                      ...
                      <el-col :span="12">
                        <el-form-item label="申请人">{{tabs.data_one.user_name}}</el-form-item>
                      </el-col>
                      <el-col :span="12">
                        <el-form-item label="申请人手机号码">{{tabs.data_one.mobile_no}}</el-form-item>
                      </el-col>
                      <el-col :span="12">
                        <el-form-item label="事件来源">{{tabs.data_one.event_source}}<span v-if="tabs.data_one.event_source_other">({{tabs.data_one.event_source_other}})</span></el-form-item>
                      </el-col>
                      <el-col :span="12">
                        <el-form-item label="事件类型">{{tabs.data_one.event_type}}<span v-if="tabs.data_one.event_type_other">({{tabs.data_one.event_type_other}})</span></el-form-item>
                      </el-col>
                      <el-col :span="24">
                        <el-form-item label="事件描述">{{tabs.data_one.event_desc}}</el-form-item>
                      </el-col>
                      <el-col :span="12">
                        <el-form-item label="需求来源">{{tabs.data_one.apply_dept_name}}-{{tabs.data_one.apply_name}}</el-form-item>
                      </el-col>
                      <el-col :span="12">
                        <el-form-item label="申请日期">{{tabs.data_one.create_time}}</el-form-item>
                      </el-col>
                      <el-col :span="12">
                        <el-form-item label="期望完成日期">{{tabs.data_one.finish_time}}</el-form-item>
                      </el-col>
                      <el-col :span="12">
        ...
        <el-row :md="24" :gutter="20">
          ...
          <el-col :span="24" :md="24">
            <el-form-item label="上传附件:">
              <el-upload
                :action="`${baseUrl}/daily/upload?token=${$getToken()}&&neelId=${addEvent.form.neel_id}`"
                :on-success="uploadFileSuccessOperation"
                auto-upload
                :on-preview="uploadFileClick"
                :on-exceed="uploadFileOut"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 清空数据
      clearAddData() {
        for (let i in this.addEvent.form) {
          if(i=='eventFrom' || i=='eventType'){
            this.addEvent.form[i] = [];
          }else{
            this.addEvent.form[i] = "";
          }
        }
      ...
      searchHandle() {
        ...
        this.$axios.post("/daily/queryAllDailyEvents",params).then((res)=>{
          let data = res.data;
          if(data.code == 200){
      ...
      handleCurrentChange(val) {
        ...
        this.$axios.post("/daily/getDailyEvent", params).then((res) => {
          let data = res.data;
          if (data.code == 200) {
      ...
      eventApply() {
        this.$maskin();
        // this.addEvent.isEdit=!this.addEvent.isEdit;
        this.addEvent.isEdit=false;
        let params = new URLSearchParams();
      ...
      updateEvent(val,e,type){
        ...
        this.$axios.post("/daily/getDailyEvent", params).then((res) => {
          ...
          if (data.code == 200) {
            this.addEvent.visiable=true;
            // this.addEvent.isEdit=true;
            this.addEvent.form.mobile_no=data.result.dailyEvent.mobile_no;
            this.addEvent.form.user_id=data.result.dailyEvent.user_id;
            this.addEvent.userName=data.result.dailyEvent.user_name;
            this.addEvent.form.task_name=data.result.dailyEvent.task_name;
            this.addEvent.form.is_urgent=parseInt(data.result.dailyEvent.is_urgent);
            this.addEvent.form.finish_time=data.result.dailyEvent.finish_time;
            this.addEvent.form.event_desc=data.result.dailyEvent.event_desc;
            this.addEvent.form.eventType=this.dealCheckBox(data.result.dailyEvent.event_type);
            this.addEvent.form.eventFrom=this.dealCheckBox(data.result.dailyEvent.event_source);
            this.addEvent.form.eventFromOther=data.result.dailyEvent.event_source_other;
            this.addEvent.form.eventTypeOther=data.result.dailyEvent.event_type_other;
            this.addEvent.fromdeptArr=data.result.deptsAndUsers;
            this.addEvent.form.fromdeptId=parseInt(data.result.dailyEvent.apply_dept);
            this.operationFileList=data.result.dailyEvent.attachments;
            this.fromdeptchangeEvent(data.result.dailyEvent.apply_dept)
            // this.addEvent.form.apply_id=parseInt(data.result.dailyEvent.apply_id);
            this.addEvent.form.apply_id=data.result.dailyEvent.apply_id;
      ...
      OperationCheck(){
        ...
        params.append("type", 'DAILY_EVENT');

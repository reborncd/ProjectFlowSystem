        ...
        <div class="content">
          ...
          <div class="table-list">
            <el-table :data="table.tableData" border style="width: 100%"
                  ...
                  ref="jsxq_table"
                  :default-sort = "{prop: 'tech_NEET_ID', order: 'descending'}"
                  @row-click="handleCurrentChange">
              <el-table-column sortable prop="tech_NEET_ID" label="需求编号" width="200" show-overflow-tooltip></el-table-column>
                ...
                <div class="console-tab-content">
                  <el-form label-width="120px" label-position="left">
                    <el-row :gutter="20">
                      ...
                      <el-col :span="24" v-if="tabs.tabsData.type === '技术需求-BUG修复'">
                        <el-form-item label="关联BUG">
                          <a href=javascript:void(0)
                            v-for="(item, index) in tabs.tabsData.selectBugList"
                            :key="item.ID"
                            @click="$go(`/home/buglist?bugId=${item.ID}`, {bugId:item.ID,neelId:tabs.tabsData.tech_NEET_ID}, {bugId:item.ID,neelId:tabs.tabsData.tech_NEET_ID})"
                            :title="item.ID">
                            {{`${index + 1}. ${item.TITLE}`}}</br>
                          </a>
                          <span v-if="tabs.tabsData.selectBugList.length === 0">无</span>
                        </el-form-item>
                      </el-col>
                      <el-col :span="12">
    ...
    <el-dialog :close-on-click-modal="false" :title="addneeds.addform.title" :visible="addneeds.addvisible" width="80%"
      ...
      <el-form label-width="100px">
        <el-row :md="24" :gutter="20">
          ...
          <el-col :span="12" :md="12" v-if="addneeds.addform.needstype == '技术需求-BUG修复'">
            <el-form-item label="关联BUG">
              <el-select multiple v-model="addneeds.addform.bugCodes" clearable collapse-tags
                     placeholder="关联的BUG" style="width: 100%">
                <el-option :key="index" v-for="(item, index) in addneeds.addform.codeArr" :label="item.title"
...
//        let card_header_height = document.querySelector(".el-card__header").offsetHeight;
      ...
      setTableData(data){
        ...
        if(this.$route.query.neelId || this.$route.params.neelId && !this.table.hasGo){
          let id;
          if (this.$route.query.neelId) {
            id = this.$route.query.neelId;
          }
          else{
            id = this.$route.params.neelId;
          }
      ...
      //改变需求类型选择需求编号
      changeNeedsType(){
        if(this.addneeds.addform.title === '编辑需求')return;
        ...
        if (this.addneeds.addform.needstype) {
          ...
          let params = new URLSearchParams();
          let arr = this.addneeds.addform.needstypeArr;
          let typeId = "";
          arr.forEach((value,index)=>{
            if(value.neel_TYPE_NAME == this.addneeds.addform.needstype){
              typeId = value.neel_TYPE_ID
            }
          })
          params.append("NEEL_TYPE_ID", typeId);
          this.$axios.post("/tech/technologyID", params).then((res) => {
            let data = res.data;
            if (data.code == 200) {
              this.addneeds.addform.code = data.result.rt;
              if(data.result.data){
                this.addneeds.addform.codeArr = [];
                this.addneeds.addform.codeArr = cloneDeep(data.result.data);
                console.log(this.addneeds.addform.codeArr);
              }
            }
          })
        }
...
//        this.addneeds.addform.fromdeptroleArr = [];
      ...
      subaddForm(){
        ...
        let params = new URLSearchParams();
        if(this.addneeds.addform.needstype == '技术需求-BUG修复'){
          //if(this.addneeds.addform.bugCodes.length === 0){
          //  this.$warn("请选择关联BUG");
          //  return;
          //}
          params.append("faultIds", JSON.stringify(this.addneeds.addform.bugCodes));	//需求编码
        }
        ...
        params.append("USER_ID", this.userInfo.user_ID);//申请人名id
        //------------------新增
      ...
      loadTabsData(val){
        ...
        this.$axios.post("/tech/queryBaseMessage", params).then((res) => {
          ...
          if (data.code == 200) {
            ...
            //--------------------------加载展示数据
            this.tabs.tabsData.selectBugList = data.result.selectBugList;
...
//            type:"category",
      ...
      consoleActionEvent(val, scope=undefined, e=undefined){
        ...
        if(!this.tabs.activeTableInfo.tech_NEET_ID) this.tabs.activeTableInfo.tech_NEET_ID = scope.tech_NEET_ID;
        ...
        switch (val.type) {
          ...
          case 'POSTPONE_REVIEW':
            // 延后评审
            val.url = 'tech/postponeReview';
            this.taskClick(val);
            break;
          case 'INVALID':
            // 无效
            val.url = 'tech/invalid';
            this.taskClick(val);
            break;
        }
      },
      taskClick(val){
        ...
        this.$confirm(`执行 ${val.name} 操作, 是否继续?`, '提示', {
          ...
          this.$maskin();
          let params = new URLSearchParams();
          params.append("TECH_NEEL_ID", this.tabs.activeTableInfo.tech_NEET_ID);
      ...
      editNeed(row){
        this.$maskin();
        let params = new URLSearchParams();
        params.append("BASE_ID",row? row.tech_NEET_ID: this.tabs.activeTableInfo.tech_NEET_ID);
        this.$axios.post("/tech/updateBaseFront",params).then((res)=>{
          ...
          if(data.code == 200){
            ...
            let lower_arr = [];
            data.result.baseTypeName.forEach((value,index)=>{
              lower_arr.push({
                'neel_TYPE_NAME':value.NEEL_TYPE_NAME,
                'neel_TYPE_ID':value.NEEL_TYPE_ID
              })
            })
            this.addneeds.addform.needstypeArr = lower_arr;//需求类型
            ...
            if(data.result.allBugList){
              this.addneeds.addform.codeArr = data.result.allBugList;
              this.addneeds.addform.bugCodes = Array.from(data.result.selectBugList, item => item.ID);
            }
            //显示弹窗
            this.addneeds.addvisible = true;
            this.$maskoff();
...
//        this.addneeds.addform.needsname = info.neel_DESCRIPTION;//需求描述
      ...
      getAssign(){
        ...
        this.myurl = "/tech/queryUserByDemand";
...
//      },
      ...
      splitTaskFinish(index, val,type){
        ...
        params.append("neelId",val.tech_NEET_ID);
...
//            }
      ...
      pageHandler(handlerType){
        ...
        if(handlerType === 'home'){
          ...
          params.append('page',this.currentPageNumber)
          params.append('pageSize',this.defaultPageSize)
          return this.$axios.post("/tech/queryAll",params).then((res)=>{
            let data = res.data;
            if(data.code == 200){
              this.current_user_id = data.result.current_user_id;

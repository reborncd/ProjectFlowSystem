<template>
  <div class="jcjs common-card-wrap"
      ...
      <div class="text item">
        <div class="content">
          <div class="action clear">
            <el-button type="danger"  size="mini" @click="dataHandle">新增</el-button>
            <!-- <el-button type="success"  size="mini" @click="powerApply">权限申请</el-button>
            <el-button type="info"  size="mini" @click="eventApply">事件受理</el-button> -->
          ...
          <div class="table-list">
            <el-table :data="table.tableData" border style="width: 100%" v-if="table.tableHeight"
                      ...
                      :default-sort = "{prop: 'daliy_NEET_ID', order: 'descending'}"
                      highlight-current-row :row-class-name="tableRowClassName" @row-click="handleCurrentChange">
              <el-table-column sortable prop="daliy_NEET_ID" label="任务编号" width="200" show-overflow-tooltip></el-table-column>
              <el-table-column sortable prop="task_NAME" label="任务名称" width="200" show-overflow-tooltip></el-table-column>
              <el-table-column sortable prop="start_DATE" label="申请日期" width="110"></el-table-column>
              <!--<el-table-column prop="start_TIME" label="申请时间" width="110"></el-table-column>-->
              <el-table-column sortable prop="end_DATE_STRING" label="期望完成日期" width="140"></el-table-column>
              ...
              <el-table-column label="操作" width="200">
                <template slot-scope="scope" class="action-wrap">
                  <!-- 曹俊卿通过和驳回 -->
                  <el-button v-if="scope.row.check_TYPE=='PRODUCT_MANAGER_CHECK'" @click="agreeRow(scope.row,$event)" size="small"
                             type="primary">通过
                  </el-button>
                  <el-button v-if="scope.row.check_TYPE=='PRODUCT_MANAGER_CHECK'" @click="unagreeRow(scope.row,$event,'驳回')"
                             size="small" type="danger">驳回
                  </el-button>
                  <el-button v-if="scope.row.check_TYPE=='DEVELOP_MANAGER_CHECK'" @click="assignRow(scope.row,$event)" size="small"
                             type="primary">分配
                  </el-button>
                  <el-button v-if="scope.row.check_TYPE=='DEVELOP_MANAGER_CHECK'" @click="unagreeRow(scope.row,$event,'驳回')"
                             size="small" type="danger">驳回
                  </el-button>
                  ...
                  <el-button v-if="scope.row.check_TYPE=='APPLY_RECOMMIT'" @click="withDraw(scope.row,$event,'撤回')"
                             size="small" type="danger">撤回
                  </el-button>
              ...
              <el-tab-pane label="详情页" name="info">
                <div class="console-tab-content">
                  <el-form label-width="150px" label-position="left">
                    <el-row :gutter="20">
                      <!--<el-col :span="24">-->
                        <!--<el-form-item v-if="tabs.data_one.daliy.design_SYSTEM" label="涉及系统">-->
                          <!--{{tabs.data_one.daliy.design_SYSTEM}}-->
                        <!--</el-form-item>-->
                      <!--</el-col>-->
                      <el-row :span="12">
                        <div style="text-align:center;color:#000;font-size:18px;font-weight:bold;">需求详情</div>
                      </el-row>
                      <el-col :span="12">
                      ...
                      <!-- <el-col :span="12" v-if="tabs.data_one.daliy.reject_RESON && tabs.data_one.daliy.state_ID==212">
                        <el-form-item label="不通过原因" style="color: red">{{tabs.data_one.daliy.reject_RESON}}
                        </el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="tabs.data_one.daliy.reject_RESON && tabs.data_one.daliy.state_ID==209">
                        <el-form-item label="通过原因" style="color: red">{{tabs.data_one.daliy.reject_RESON}}
                        </el-form-item>
                      </el-col> -->
                      <el-col :span="12">
                      ...
                      <el-col :span="12">
                        <el-form-item label="期望完成日期">{{tabs.data_one.daliy.end_DATE_STRING}}</el-form-item>
                      </el-col>
                      <el-col :span="12">
                      ...
                      <el-col :span="12" v-if="tabs.data_one.daliy.reviewLevel">
                        <el-form-item label="评审等级">{{tabs.data_one.daliy.reviewLevel}}</el-form-item>
                      </el-col>
                      <!-- <el-col :span="24">
                        <el-form-item label="产品改造点">
                          <div class="prduct_FUNCTION"></div>
                        </el-form-item>
                      </el-col> -->
                      <el-col :span="12" v-if="tabs.data_one.daliy.reviewResult">
                        <el-form-item label="需求评审结果 ">{{tabs.data_one.daliy.reviewResult}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="tabs.data_one.daliy.reviewTime">
                        <el-form-item label="评审时间 ">{{tabs.data_one.daliy.reviewTime}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="tabs.data_one.daliy.attachment_id && tabs.data_one.daliy.attachment_name">
                        <el-form-item label="附件">
                          <template>
                            <span :key="index" style="color: #409EFF; cursor:pointer;margin-right:10px;" @click="downloadAtt(tabs.data_one.daliy.attachment_id)">{{tabs.data_one.daliy.attachment_name}}</span>
                      ...
                      <el-col :span="12" v-if="tabs.data_one.daliy.reject_RESON">
                        <el-form-item label="驳回原因 "><span style="color:red;">{{tabs.data_one.daliy.reject_RESON}}</span></el-form-item>
                      </el-col>
                  ...
                  <el-form label-width="150px" label-position="left" v-if="tabs.data_one.developInfos.length" v-for="(item,index) in tabs.data_one.developInfos" :key="index">
                    <el-row :gutter="20">
                      <el-row :span="12">
                        <div style="text-align:center;color:#000;font-size:18px;font-weight:bold;">开发详情</div>
                      </el-row>
                      <el-col :span="12" v-if="item.develop_NAME && item.develop_NAME!=='null' && item.develop_NAME!=='undefined'">
                        <el-form-item label="开发人员:">{{item.develop_NAME}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.attachment_ID">
                        <el-form-item label="开发附件:">
                          <template>
                            <div style="color: #409EFF; cursor:pointer;" @click="downloadAtt(item.attachment_ID)">{{item.attachment_NAME}}</div>
                          </template>
                        </el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.actual_WORK_TIME && item.actual_WORK_TIME!=='null' && item.actual_WORK_TIME!=='undefined'">
                        <el-form-item label="实际工时:">
                          <span style="color:red;">{{item.actual_WORK_TIME}}</span>
                        </el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.ori_SYSTEM_NAME && item.ori_SYSTEM_NAME!=='null' && item.ori_SYSTEM_NAME!=='undefined'">
                        <el-form-item label="数据源-系统名称:">{{item.ori_SYSTEM_NAME}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.ori_SYSTEM_IP && item.ori_SYSTEM_IP!=='null' && item.ori_SYSTEM_IP!=='undefined'">
                        <el-form-item label="数据源-IP地址:">{{item.ori_SYSTEM_IP}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.ori_DATA_TYPE || item.ori_OTHER_DATA_TYPE">
                        <el-form-item label="数据源-数据类型:">{{item.ori_DATA_TYPE=='其它'?item.ori_OTHER_DATA_TYPE:item.ori_DATA_TYPE}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.ori_DATA_PATH && item.ori_DATA_PATH!=='null' && item.ori_DATA_PATH!=='undefined'">
                        <el-form-item label="数据源-数据路径:">{{item.ori_DATA_PATH}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.ori_SENSITIVE_DATA && item.ori_SENSITIVE_DATA!=='null' && item.ori_SENSITIVE_DATA!=='undefined'">
                        <el-form-item label="数据源-敏感信息:">{{item.ori_SENSITIVE_DATA}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.is_HIDDEN_SENSITIVE_DATA && item.is_HIDDEN_SENSITIVE_DATA!=='null' && item.is_HIDDEN_SENSITIVE_DATA!=='undefined'">
                        <el-form-item label="数据源-是否脱敏:">{{item.is_HIDDEN_SENSITIVE_DATA}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.hidden_WAY && item.hidden_WAY!=='null' && item.hidden_WAY!=='undefined'">
                        <el-form-item label="数据源-脱敏方式:">{{item.hidden_WAY}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.ori_REMARKS && item.ori_REMARKS!=='null' && item.ori_REMARKS!=='undefined'">
                        <el-form-item label="数据源-其他说明:">{{item.ori_REMARKS}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.des_SYSTEM_NAME && item.des_SYSTEM_NAME!=='null' && item.des_SYSTEM_NAME!=='undefined'">
                        <el-form-item label="数据目标-系统名称:">{{item.des_SYSTEM_NAME}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.des_SYSTEM_IP && item.des_SYSTEM_IP!=='null' && item.des_SYSTEM_IP!=='undefined'">
                        <el-form-item label="数据目标-IP地址:">{{item.des_SYSTEM_IP}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.des_DATA_PATH && item.des_DATA_PATH!=='null' && item.des_DATA_PATH!=='undefined'">
                        <el-form-item label="数据目标-数据路径:">{{item.des_DATA_PATH}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.des_REMARKS && item.des_REMARKS!=='null' && item.des_REMARKS!=='undefined'">
                        <el-form-item label="数据目标-其他说明:">{{item.des_REMARKS}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.risk_LEVEL && item.risk_LEVEL!=='null' && item.risk_LEVEL!=='undefined'">
                        <el-form-item label="风险等级:">{{item.risk_LEVEL}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.risk_DESC && item.risk_DESC!=='null' && item.risk_DESC!=='undefined'">
                        <el-form-item label="风险说明:">{{item.risk_DESC}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="item.risk_MEASURE && item.risk_MEASURE!=='null' && item.risk_MEASURE!=='undefined'">
                        <el-form-item label="控制措施:">{{item.risk_MEASURE}}</el-form-item>
                      </el-col>
                    </el-row>
                    </el-form>
                    <el-form label-width="150px" v-if="tabs.data_one.daliy.confirm_NAME" label-position="left">
                      <el-row :span="12">
                        <div style="text-align:center;color:#000;font-size:18px;font-weight:bold;">运维详情</div>
                      </el-row>
                      <el-col :span="12" v-if="tabs.data_one.daliy.confirm_NAME">
                        <el-form-item label="运维执行人员:">{{tabs.data_one.daliy.confirm_NAME}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="tabs.data_one.daliy.status">
                        <el-form-item label="执行结果:">{{tabs.data_one.daliy.status=='1'?'成功':'失败'}}</el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="tabs.data_one.daliy.attachmentId">
                        <el-form-item label="运维上传附件 ">
                          <template>
                            <div style="color: #409EFF; cursor:pointer;" @click="downloadAtt(tabs.data_one.daliy.attachmentId)">{{tabs.data_one.daliy.attachmentName}}</div>
                          </template>
                        </el-form-item>
                      </el-col>
                      <el-col :span="12" v-if="tabs.data_one.daliy.remarks">
                        <el-form-item label="运维备注:">{{tabs.data_one.daliy.remarks}}</el-form-item>
                      </el-col>
                    </el-form>
                </div>
              </el-tab-pane>
              ...
              <el-tab-pane label="操作台" name="console">
                <div class="console-tab-content">
                  ...
                  <el-form v-if="tabs.data_one.daliy.check_TYPE=='TECH_MANAGER_SPLIT'" label-width="120px" label-position="right">
                    ...
                    <el-row :span="24">
                      <el-col :span="24" style="text-align: center;position: relative;margin-bottom: 20px">
                        拆分任务：{{addneeds.addform.splitArr.length}}
                        <el-button type="primary" @click="distributionTask1" size="mini"
                                   style="position: absolute;right: 0;">分配任务
                        </el-button>
                      </el-col>
                    </el-row> -->
                    <el-col :span="12" :md="12">
                    ...
                    <!-- <div class="table-list">
                      <el-table :data="addneeds.addform.splitArr" border style="width: 100%" height="200" show-summary
                                highlight-current-row>
                        <el-table-column prop="USER_NAME" label="人员"></el-table-column>
                        <el-table-column prop="xtname" label="任务编码" show-overflow-tooltip></el-table-column>
                        <el-table-column prop="fzmk" label="负责模块" show-overflow-tooltip></el-table-column>
                        <el-table-column prop="willTime" label="预计用时(小时)" width="160"></el-table-column>
                  ...
                  <div class="tab-console-handler">
                      <el-button v-if="tabs.data_one.daliy.check_TYPE=='DATA_CENTER_EXECUTE'" @click="OperationCheck" size="medium" type="primary">确认</el-button>
                      <el-button v-if="tabs.ifSplit" @click="createNewN" size="medium" type="primary">提交 </el-button>
                      <el-button v-if="tabs.data_one.daliy.check_TYPE=='DEVELOP_MANAGER_CHECK'" @click="assignPeople" size="medium" type="primary">分配人员</el-button>
                      <el-button v-if="tabs.data_one.daliy.check_TYPE=='DEVELOP_MANAGER_CHECK'" @click="checkDaliy" size="medium" type="primary">提交 </el-button>
                      <el-button v-if="tabs.data_one.daliy.check_TYPE=='TECH_MANAGER_SPLIT'" @click="checkDaliyTech" size="medium" type="primary">提交分析结果</el-button>
                      <el-button v-if="tabs.data_one.daliy.check_TYPE=='DEVELOPING'" @click="checkDaliyDev" size="medium" type="primary">完成</el-button>
                      <el-button v-if="tabs.data_one.daliy.check_TYPE=='REJECT_AND_UPDATE_DEVELOP'" @click="checkDaliyDevUpdate" size="medium" type="primary">重新提交</el-button>
                      <el-button v-if="tabs.ifReject" @click="unagreeRow(tabs,$event,'驳回')" size="medium" type="primary">驳回</el-button>
                      ...
                      <el-button v-if="tabs.data_one.daliy.check_TYPE=='QUALITY_DEPARTMENT_CHECK'" @click="assignOperation" size="medium" type="primary">分配</el-button>
                      <el-button v-if="tabs.data_one.daliy.check_TYPE=='QUALITY_DEPARTMENT_CHECK'" @click="QualityCheck" size="medium" type="primary">通过</el-button>
                      <el-button
                  ...
                  <div class="table-list" style="margin-top: 40px;">
                    <el-table :data="tabs.data_one.infos" show-summary border style="width: 100%" highlight-current-row>
                      ...
                      <el-table-column label="操作" width="130">
                        <template slot-scope="scope" class="action-wrap">
                          <el-button v-if="scope.row.state_ID==205" @click="startTask(scope.row,scope,$event)"
                                     size="middle" type="primary">开始
                          </el-button>
                          <el-button v-if="scope.row.state_ID==206" @click="finishTask(scope.row,scope,$event)"
                                     size="middle" type="primary">完成
                          </el-button>
              ...
              <el-tab-pane label="拆分项目" name="split">
                <div class="console-tab-content">
                  <div class="table-list" v-if="tabs.data_one.deptSystem">
                    <el-table :data="tabs.data_one.deptSystem" show-summary border style="width: 100%"
                              highlight-current-row @row-click="splitTaskClick">
    ...
    <!--新建任务-->
    <!-- <el-dialog title="技术经理新建任务" :visible="addneeds.addvisible" width="60%" append-to-body modal-append-to-body
      ...
      <el-form :model="addneeds.addform" ref="addneeds.addform" label-width="100px" class="demo-ruleForm">
        ...
        <el-row :span="24">
          <el-col :span="24" style="text-align: center;position: relative;"> 拆分任务：{{addneeds.addform.splitArr.length}}
            <el-button type="primary" @click="distributionTask" size="mini" style="position: absolute;right: 0;">分配任务
            </el-button>
          </el-col>
        </el-row>
        <el-row :span="24" style="margin-top: 20px;">
          <el-col :span="12" :md="12">
            <el-form-item label="预计开始时间">
              <el-date-picker type="date" placeholder="选择日期" @change="changeEnd" v-model="addneeds.addform.startTime"
                              style="width: 100%;"></el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="12" :md="12">
            <el-form-item label="预计完成时间">
              <el-date-picker type="date" v-if="!addneeds.addform.startTime" disabled :picker-options="pickerOptions"
                              placeholder="选择日期" v-model="addneeds.addform.endTime"
                              style="width: 100%;"></el-date-picker>
              <el-date-picker type="date" v-if="addneeds.addform.startTime" :picker-options="pickerOptions"
                              placeholder="选择日期" v-model="addneeds.addform.endTime"
        ...
        <el-row :md="24" :gutter="20">
          ...
          <el-col :span="24" :md="24">
            <el-form-item label="上传附件:">
              <el-upload
                :limit="1"
                :action="`${baseUrl}/daily/upload?token=${$getToken()}&&DALIY_NEET_ID=${handle.daliy_NEET_ID}`"
                :on-success="uploadFileSuccessAdd"
                auto-upload
                :on-preview="uploadFileClick"
                :on-exceed="uploadFileOut"
                :on-remove="uploadClearOperation"
        ...
        <el-row :md="24" :gutter="20">
          ...
          <el-col :span="12" :md="12">
            <el-form-item label="申请时间">
              <el-radio v-model="addneeds.addform.jiaji" label="0">永久</el-radio>
              <el-radio v-model="addneeds.addform.jiaji" label="1">临时</el-radio>
            </el-form-item>
          </el-col>
          <el-col :span="24" style="width:100%;" :md="12">
        ...
        <el-row :md="24" :gutter="20">
          ...
          <el-col :span="24" style="width:100%;" :md="12">
            <el-form-item label="事件描述:">
              <el-input clearable maxlength="1000" v-model="addEvent.form.event_desc"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      searchHandle() {
        ...
        this.$axios.post("/daily/queryAllDaily",params).then((res)=>{
          let data = res.data;
          if(data.code == 200){
      ...
      pageHandler(handlerType){
        ...
        if(handlerType === 'home'){
          ...
          return this.$axios.post("/daily/queryAllDaily",params).then((res)=>{
            let data = res.data;
            if(data.code == 200){
            this.setTableData(data.result.daliys);
              for (let i of data.result.state) {
      ...
      handleCurrentChange(val) {
        ...
        this.$axios.post("/daily/queryDailyMessage", params).then((res) => {
          ...
          if (data.code == 200) {
            ...
            this.$set(this.tabs, "checksArr", data.result.reviewResult);
      ...
      createNew2() {
        if (this.proFilter() == "1") {
          ...
          if(this.addFileList.length>0){
            params.append("attachmentIds", this.addFileList[0].id); //附件
          }
      ...
      QualityCheck() {
        ...
        params.append("DAILY_NEET_ID", this.handle.daliy_NEET_ID);
        this.$axios.post("/daily/qualityDeptAssignTask", params).then((res) => {
          let data = res.data;
          if (data.code == 200) {
            this.$success("操作成功");
            this.loadData();
      ...
      OperationCheck(){
        ...
        let params = new URLSearchParams();
        params.append("DAILY_NEET_ID", this.handle.daliy_NEET_ID); //任务编号
        params.append("remarks", this.operationRemarks); //任务编号
        if(this.operationFileList.length>0){
          params.append("attachmentId", this.operationFileList[0].id); //任务编号
        }
        params.append("status", this.operationResult); //任务编号
      ...
      unagreeRow(val, e, text,type) {
        ...
        this.$prompt('确定进行' + text + '操作？', '请填写' + text + '理由', {
          ...
          let params = new URLSearchParams();
          params.append('DALIY_NEET_ID', val.daliy_NEET_ID);
          type?params.append('STATE_ID', '107'):params.append('STATE_ID', '100');//驳回
          params.append('REJECT_RESON', value);
          this.$axios.post("/daily/updateDaily", params).then((res) => {
            let data = res.data;
            if (data.code == 200) {
